...
| Key | Purpose |
|---|---|
| `Resend:ApiKey` | Transactional email (password reset, etc.) via [Resend](https://resend.com/) |
| `Email:FromAddress`, `Email:FrontendBaseUrl` | Email sender address and link target for the member frontend |
| `Search:Host`, `Search:AdminKey` | Azure AI Search endpoint/key used for the parts search index |
| `AppSettings:Secret` | Symmetric key used to sign JWT access tokens |
