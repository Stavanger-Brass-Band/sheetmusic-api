...
namespace SheetMusic.Api.Repositories
{
    public class ProjectRepository : IProjectRepository
    {
        ...
        public async Task<Project> ResolveByIdentifierAsync(string identifier)
        {
            ...
            if (Guid.TryParse(identifier, out var guid))
            {
                result = await context.Projects
                    .FirstOrDefaultAsync(project => project.Id == guid);
            }
            else
            {
                //ignore casing when comparing on title
                result = await context.Projects
                    .SingleOrDefaultAsync(project => project.Name.ToLower() == identifier.ToLower());
            }

            if (result == null) throw new NotFoundError($"projects/{identifier}", "Project not found");
        ...
        public async Task DeleteProjectAsync(string projectIdentifier)
        {
            ...
            if (project == null) throw new NotFoundError($"projects/{projectIdentifier}");
